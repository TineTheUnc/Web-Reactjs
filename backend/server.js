...
app.post("/login", jsonParser, async (req, res) => {
    if (req.body.Secret == secret) {
        const DB = await pool.getConnection();
        await DB.beginTransaction();
        await DB.query("SELECT * FROM user WHERE Email = ? ", [req.body.Email])
            .then(async ([result]) => {
                await DB.commit();
                await DB.release();
                if (result.length == 0) {
...
app.post("/getUser", jsonParser, async (req, res) => {
    ...
    if (!TOKEN) {
        ...
        try {
            ...
            const DB = await pool.getConnection();
            await DB.beginTransaction();
            await DB.query("SELECT * FROM user WHERE ID = ? AND Email = ?", [
                data.id,
                data.email,
            ])
                .then(async ([result]) => {
                    ...
                    if (result.length == 0) {
                        ...
                        await fs
                            .readFile(result[0].Avatar, "base64")
                            .then(async (data) => {
                                result[0].Avatar =
                                    "data:image/" +
                                    path.extname(result[0].Avatar).split(".")[1] +
                                    ";base64," +
                                    data;
                                return res.json({
...
app.post("/getPasslist", jsonParser, async (req, res) => {
    ...
    if (!TOKEN) {
        ...
        const DB = await pool.getConnection();
        await DB.beginTransaction();
        await DB.query("SELECT * FROM passlist WHERE user_ID = ?", [data.id])
            .then(async ([result]) => {
                await DB.commit();
                await DB.release();
                if (result.length == 0) {
...
app.post("/addPasslist", jsonParser, async (req, res) => {
    ...
    if (!TOKEN) {
        ...
        if (Object.keys(req.body).toString() == "Key,Password,Secret,Cookies") {
            ...
            await DB.beginTransaction();
            await DB.query(
                "INSERT INTO passlist (`Key`,`Password`,`user_ID`) VALUES (?,?,?)",
                [req.body.Key, req.body.Password, data.id]
            )
                .then(async () => {
                    await DB.commit();
                    await DB.release();
...
app.post("/updateUser", async (req, res) => {
    ...
    if (!TOKEN) {
        ...
        try {
            ...
            await fs
                .writeFile(filename, base64Image, { encoding: "base64" })
                .then(async () => {
                    const DB = await pool.getConnection();
                    await DB.beginTransaction();
                    await DB.query("SELECT * FROM user WHERE ID = ? AND Email = ?", [
                        ...
                        .then(async ([result]) => {
                            if (result.length == 0) {
                                ...
                                await DB.query(
                                    "UPDATE user SET FirstName = ?, LastName = ?, Avatar = ? WHERE ID = ?",
                                    [req.body.FirstName, req.body.LastName, filename, data.id]
                                )
                                    .then(async () => {
                                        await DB.commit();
                                        await DB.release();
            ...
            await DB.query("SELECT * FROM user WHERE ID = ? AND Email = ?", [
                ...
                .then(async ([result]) => {
                    ...
                    if (result.length == 0) {
                        ...
                        await DB.query("DELETE FROM user WHERE ID = ?", [data.ID])
                            .then(() => {
...
app.get("/default_avatar", async (req, res) => {
    await fs
        .readFile("avatars\\default_avatar.jpg", "base64")
        .then(async (data) => {
            const avatar =
                "data:image/" +
                path.extname("avatars\\default_avatar.jpg").split(".")[1] +
                ";base64," +
                data;
